...
<template>
    ...
    >
        ...
        <VaTabs v-model="addtionalTabValue" color="info" vertical grow>
            ...
            <div class="text-xl font-bold">
                {{
                    currentTab(addtionalTabValue, additionalSyntaxTabs).value
                        .title
                }}
            </div>
            <VaDivider />
    ...
    >
        ...
        <VaTabs v-model="statusTabValue" color="info" vertical grow>
            <template #tabs>
                ...
                >
                    ...
                    <div v-if="tab.title === '未生效'">
                        <div :class="`inline !text-[${statusColorSet[4]}]`">
                            ▲
                        </div>
                        未生效
                    </div>
                    <div
                        v-if="
                            tab.title != '未生效' && tab.title != '四大完成狀態'
                        "
                    >
                        {{ tab.title }}
                    </div>
            ...
            <div
                v-html="
                    currentTab(statusTabValue, statusSyntaxTabs).value.content
                "
                class="text-xl mt-2"
            />
            <!-- 四大完成狀態的獨立顯示 -->
