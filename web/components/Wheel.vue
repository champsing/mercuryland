...
if (sessionStorage.getItem("wheelConnect")) {
  ...
  axios.get(BASE_URL + "/api/wheel/create").then(
    (response) => {
      ...
      sessionStorage.setItem("wheelConnect", JSON.stringify(wheelConnect));
    },
    (error) => {
      wheelConnect.id = -1;
      console.log(error);
    ...
    <div class="flex w-full justify-evenly">
      ...
      <div class="w-1/5">
        ...
        <VaButton
          ...
          :disabled="
            isSpinning || count(textArea2) == 0 || wheelConnect.id <= 0
          "
        >
          完成抽選
        </VaButton>
        <div class="text-red-500 text-right" v-if="wheelConnect.id <= 0">
          <VaDivider color="danger" orientation="right"> 停用 </VaDivider>
        </div>
