use crate::{config::CONFIG, database::anonymous::Anonymous, error::ServerError};
use chrono::Utc;
use poise;
...
pub async fn handle_component(
    ...
    let modal = serenity::all::CreateModal::new("anonymous_modal", "匿名發言").components(vec![
...
pub async fn handle_modal(ctx: serenity::all::Context, modal: serenity::all::ModalInteraction) {
    let content = if let serenity::all::ActionRowComponent::InputText(ref input_text) =
    ...
    let image_url = if modal.data.components.len() > 1 {
        if let serenity::all::ActionRowComponent::InputText(ref input_text) =
            modal.data.components[1].components[0]
        {
            input_text.value.clone().unwrap_or_default()
        } else {
            String::new()
    ...
    let result: Result<Result<(i64, String, String), ServerError>, tokio::task::JoinError> =
